...
# This is so that MiMe can produce a
...
# this can be done in Gaussian job with units=au
...
spejob = "#"+" "+spelevel+" "+"units=au symm=none"
gradjob = "#"+" "+gradlevel+" "+"force=nostep units=au symm=none"
hessjob = "#"+" "+hesslevel+" "+"freq=(noraman,noprintnm) units=au symm=none"
...
if runforce:
    ...
    #subprocess.run(formchk+" "+grad_chkfile, shell=True)
...
#gradfile = grad_fchkfile
#hessfile = freq_outfile
hessfile = freq_fchkfile
...
if gradfile == grad_outfile: gradout = True
elif gradfile == freq_fchkfile: gradout = False
...
if deriv >= 1:
    ...
    # Extract Forces
    # if from fchk file F = dE/dX (F = grads)
    # if from out file F = -dE/dX (F = force)
    if not gradout: F = -parser.parse().grads.reshape(1,natoms,3)
    else: F = parser.parse().grads
if deriv == 2:
...
with open(Eout,"w") as OUTP:
    ...
    if deriv >= 1:
        ...
        # It is very important that F[-1] is used, not F[0]
        for row in -F[-1]:
            OUTP.write(str("{:20.12e}"*3).format(*row)+"\n")
